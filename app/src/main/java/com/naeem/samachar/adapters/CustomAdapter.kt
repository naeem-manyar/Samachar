...
class CustomAdapter(private var newsList: List<NewsModel>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (context.toString().contains("SavedNews")) {
            ...
            val currentTimeInHours = DateTime.now(DateTimeZone.forID("Asia/Kolkata"))
            val newsTimeInHours = DateTime.parse(time).withZone(DateTimeZone.forID("Asia/Kolkata"))
            val hoursDifference = Duration(currentTimeInHours, newsTimeInHours)
            val hoursAgo = " " + hoursDifference.standardHours.hours.toString().substring(1) + " ago"
